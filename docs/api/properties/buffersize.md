...
Gets or sets the buffer’s size, x10 MB, for text stream operations.
{: .fs-6 .fw-300 }
...
>The `bufferSize` enable user to specify how many data is read at a time. By default, the `bufferSize` property is set to 0.5 (5 MB). For files holding very long lines, user must to choose a size enough to hold couple of lines at once.
